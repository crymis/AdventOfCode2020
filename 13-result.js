...
var input = fs_1.readFileSync('./13-input.txt', { encoding: 'ascii' });
var testInput = "939\n7,13,x,x,59,x,31,19";
var testInput2 = "000\n67,7,59,61";
var testInput3 = "000\n67,x,7,59,61";
var testInput4 = "000\n67,7,x,59,61";
var testInput5 = "000\n1789,37,47,1889";
var inputInUse = input;
var earliestTimestamp = Number(inputInUse.split('\n')[0]);
var busIdsWithX = inputInUse.split('\n')[1].split(',').map(function (x) { return x === 'x' ? undefined : Number(x); });
var busIds = busIdsWithX.filter(function (x) { return typeof x === 'number'; });
var getClosestTimestamp = function (busIds, timestamp) {
    var minTime = Number.MAX_VALUE;
    var minTimeBusId = 0;
    busIds.forEach(function (busId) {
        var closestMultiplier = Math.ceil(timestamp / busId);
        var waitingTime = (busId * closestMultiplier) - timestamp;
...
var _a = getClosestTimestamp(busIds, earliestTimestamp), id = _a[0], waitingTime = _a[1];
...
var hasValInCommon = function (busIds) {
    var results = {};
    busIds.forEach(function (x, j) {
        for (var i = 0; i < busIds.length; i = j + i + x) {
    ...
    if (Object.keys(results).length > 0)
        multiplicator = Number(Object.keys(results)[0]) * results[Object.keys(results)[0]].split(',').reduce(function (acc, x) { return acc *= Number(x); }, 1);
...
// kgv(23, 647) =      14881
var findEarliestTimestampWithPattern = function (busIds) {
    var timestampOffset = 99999999808657; //(100000000000000 / kgv)
    var kgv = hasValInCommon(busIds);
    var failed = false;
    var foundTs = undefined;
    console.log(busIds, kgv);
    for (var ts = 0; foundTs === undefined; ts += kgv) {
        failed = false;
        ...
        for (var k = 0; k < busIds.length; k++) {
            if (!busIds[k])
                continue;
            var indexDiff = (Math.abs(k - (busIds.length - 1)));
