...
import { addEmitHelper, textSpanContainsTextSpan } from 'typescript'
...
const input = readFileSync('./10-input.txt', { encoding: 'ascii' })
const testInput = `16
10
15
5
...
10
3`
const getFormattedArray = (inp) => {
  const adapterArray = inp.split('\n').map(x => Number(x)).sort((a, b) => a - b)
...
const testAdapterArray = getFormattedArray(testInput)
const testAdapterArray2 = getFormattedArray(testInput2)
const adapterArray = getFormattedArray(input)

const diffArray = adapterArray.map((jolts, i) => adapterArray[i + 1] ? adapterArray[i + 1] - jolts : 0)

const res = diffArray.filter(x => x === 1).length * diffArray.filter(x => x === 3).length
